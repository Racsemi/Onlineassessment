...
export default function EditAssessmentWizardPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function loadData() {
      const qRes = await fetchApi('/questions?limit=100');
      if (qRes.success && qRes.data) {
        setAvailableQuestions(qRes.data);
      }

      if (id) {
        const aRes = await fetchApi(`/assessments/${id}`);
        if (aRes.success && aRes.data) {
          ...
          setTimingMode(a.timingMode);
          setDurationMinutes(a.durationMinutes);
          setPassingPercentage(a.passingPercentage);
          setProctoringMode(a.proctoringMode || (a.integrityMonitoring ? 'BASIC' : 'OFF'));
          
          if (a.sections && a.sections.length > 0) {
            setSections(a.sections.map((s: any) => ({
              title: s.title,
              durationMinutes: s.durationMinutes,
              questionIds: s.assessmentQuestions?.map((aq: any) => aq.questionId) || []
            })));
          }
        }
      }
  ...
  if (!isDataLoaded) return <div className="p-10 text-center">Loading assessment data...</div>;
  ...
  const handleSaveAndPublish = async () => {
    setLoading(true);
    const res = await fetchApi(`/assessments/${id}`, {
      method: 'PUT',
      body: JSON.stringify({
        title,
        description,
        ...
        <div className="bg-white border border-gray-200 shadow-sm rounded-2xl p-8 space-y-6">
          ...
          {currentStep === 9 && (
            <div className="text-center py-6 space-y-4">
              ...
              <h3 className="text-xl font-bold text-gray-900">Ready to Save Updates</h3>
              <p className="text-xs text-gray-500 max-w-md mx-auto">
                These changes will immediately apply to the assessment structure. (Cannot be modified if candidates have already started).
              </p>
              <button
                type="button"
              ...
              >
                {loading ? 'Saving Updates...' : 'Save Assessment Updates'}
              </button>
            </div>
          )}
